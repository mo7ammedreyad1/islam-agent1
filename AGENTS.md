...
- **مجلد `identities/` هيحتوي مع الوقت على أكتر من هوية بصرية مختلفة (استايلات
  متعددة)، مش هوية واحدة بس.** `brown-style.html` هي مجرد اسم هوية واحدة من
  ضمنهم، **مش الهوية الافتراضية العامة** — لازم تتعامل مع اسم الملف المذكور في
  المهمة بالحرف، وميصحش تفترض "زمان كنا بنستخدم brown-style فهستخدمها تاني" لو
  المهمة سمّت ملف تاني أو ملفت غير واضح.
...
- **كل هوية جديدة فيها "منطقة تعديل" محدّدة بتعليق واضح جوه الملف نفسه**
  (`SURAH_NUMBER`, `RECITER_ID`, `OUTPUT_FILENAME`, وبعدها `SURAH_VERSES`).
...
9. **حزمة `@mediabunny/aac-encoder` لازم تكون معرّفة في الـ `importmap` من الأول**
   ...
   `identities/brown-style.html`) فيها الإصلاح ده جاهز.
